/**
 * Legacy pre-P8 screens (dashboard / checkpoints / transactions / settings /
 * devices), preserved verbatim from the previous App.tsx.
 *
 * These are intentionally NOT wired into the R4-P8 primary navigation. They
 * predate the session-token bootstrap and call legacy endpoints directly; do
 * not extend them here. P8 UI work happens in src/pages/*.
 */
        ...
        {status?.checks && <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
          {Object.entries(status.checks).map(([k,v])=>
            <div key={k} style={{padding:'8px 12px',background:v?'#dcfce7':'#fee2e2',
              borderRadius:8,fontSize:14}}>{k}: {v?'PASS':'FAIL'}</div>
          )}
        </div>}
        <h2>Versions</h2>
